...
"""
    This is an updated version of the tests in sudoku-wiki.py and sudoku-wiki-dump.py
    using the input and output formats for demystify. 

    All test outputs are available in JSON format in ./sudoku-wiki-test-results and
    can be viewed using the demystify visualiser.

    NB: No literal forcing has been used, so these results are purely what demystify
    believes to be the best choice for each step.
    - Matthew McIlree
"""
def dotest(doms, name, pos):
    exp = explain.Explainer()
    p = open("./temp.param", "w")
    fixed = []
    for i, row in enumerate(doms):
        fixed.append([])
        for j, cell in enumerate(row):
            if len(cell) == 1:
                fixed[i].append(cell[0])
            else:
                fixed[i].append(0)
    p.write("letting fixed be " + str(fixed))
    p.close()

    exp.init_from_essence("../eprime/sudoku.eprime", "./temp.param")
            ...
            if len(cell) > 1:
                for value in range(1, 10):
                    if value not in cell:
                        l = exp.find_lit(i+1, j+1, value)
                        exp._add_known([l])

    res = exp.explain_steps(num_steps=2)
    f = open("./sudoku-wiki-test-results/" + name + ".json", "w")
