...
def parse_essence(eprime, eprimeparam):
    ...
    if paramjson.returncode != 0:
        raise ParseError("Conjure pretty-printing of params failed" + "\n"
                            + paramjson.stdout + "\n"
                            + paramjson.stderr)                       
    params = json.loads(paramjson.stdout)

    makedimacs = subprocess.run(
            ["savilerow", "-in-eprime", eprime, "-in-param", eprimeparam, 
            "-sat-output-mapping", "-sat", "-sat-family","lingeling","-S0",
            "-O0","-reduce-domains","-aggregate"], 
            stdout=subprocess.PIPE, stderr=subprocess.PIPE)

    if makedimacs.returncode != 0:
        raise ParseError("savilerow failed" + "\n"
                            + makedimacs.stdout + "\n"
                            + makedimacs.stderr) 
    ...
    for v in SortedSet(varmap.keys()).intersection(SortedSet(cons.keys())):
        ...
        for k in SortedSet(varmap[v].keys()):
            ...
            connected = SortedSet(
                        lit for s in 
                            demystify.utils.getConnectedVars(
            ...
            connected = connected.union(SortedSet(
                lit.neg() for lit in connected))
            
            constraintmap[demystify.base.DummyClause(
                    constraintname, connected)] = varmap[v][k][1]
    ...
    for v in printvarmap.keys():
        ...
        if dim==0:
            ...
            printvarlist.append(
                demystify.base.VarMatrix(
                    None, (len(varlist), len(varlist[0])), (), 
                    varmat = varlist))
        else:
            assert False

    puz = demystify.base.Puzzle(printvarlist)
    solver = demystify.internal.Solver(puz, cnf=formula, litmap=litmap, 
                conmap=constraintmap)
