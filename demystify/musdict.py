...
class MusDict(object):
    ...
    def has_literal(self, literal):
        return len(list(k for k in sorted(self.mus_dict.keys()) 
            if str(k) == literal)) > 0
    ...
    def update_musdict(self, p, mus):
        if mus is None:
            return

        elif p not in self.mus_dict:
            logging.info("XX found first {} {}"
                    .format(p, len(mus)))

            self.mus_dict[p] = [tuple(sorted(mus))]

        elif len(self.mus_dict[p][0]) > len(mus):
            logging.info("XX found new best {} {} {}"
                    .format(p, len(self.mus_dict[p][0]), len(mus)))

            self.mus_dict[p] = [tuple(sorted(mus))]

        elif p in self.mus_dict and len(self.mus_dict[p][0]) == len(mus):
            logging.info("XX add new best {} {} {}"
                    .format(p, len(self.mus_dict[p][0]), len(mus)))
